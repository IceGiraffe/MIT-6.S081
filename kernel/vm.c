...
uint64
uvmdealloc_wo_defree(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
  if(newsz >= oldsz)
    return oldsz;
...
int u2k_pt(pagetable_t pt, pagetable_t kpt, uint64 st, uint64 ed) {
  pte_t *u_pte, *k_pte;

  // 不能超过PLIC，内核空间底
  if ((ed < st) || (PGROUNDUP(ed) >= PLIC))
    return -1;

  for (uint64 va = PGROUNDUP(st); va < ed; va += PGSIZE)
  {
    if ((u_pte = walk(pt, va, 0)) == 0){
      panic("copyin_new: pte not exist");
    }
    if ((k_pte = walk(kpt, va, 1)) == 0){
